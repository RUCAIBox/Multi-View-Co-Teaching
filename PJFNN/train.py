...
pjfnn = PJFNN(args).to(device)

criterion = nn.BCEWithLogitsLoss()
optimizer = torch.optim.Adam(pjfnn.parameters(), lr=args['train']['lr'])
print(pjfnn)
print(get_parameter_number(pjfnn))
...
for epoch in range(args['num_epochs']):
    ...
    for i, (geek_sent, job_sent, labels) in enumerate(train_loader):
        ...
        outputs = pjfnn(geek_sent, job_sent)
    ...
    if acc > best_acc:
        ...
        torch.save(pjfnn.state_dict(), os.path.join(args['saved_path'], 'model-{}.ckpt'.format(int(epoch+1))))
...
# test
pjfnn.load_state_dict( torch.load(os.path.join(args['saved_path'], '{}-best.ckpt'.format(args['token']))) )
pjfnn.eval()
