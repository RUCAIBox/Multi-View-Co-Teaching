...
apjfnn = APJFNN(args).to(device)

criterion = nn.BCEWithLogitsLoss()
optimizer = torch.optim.Adam(apjfnn.parameters(), lr=args['train']['lr'])
print(apjfnn)
print(get_parameter_number(apjfnn))
...
for epoch in range(args['num_epochs']):
    ...
    for i, (geek_sent, job_sent, labels) in enumerate(train_loader):
        ...
        outputs = apjfnn(geek_sent, job_sent)
    ...
    if acc > best_acc:
        ...
        torch.save(apjfnn.state_dict(), os.path.join(args['saved_path'], 'model-{}.ckpt'.format(int(epoch+1))))
...
# test
apjfnn.load_state_dict( torch.load(os.path.join(args['saved_path'], '{}-best.ckpt'.format(args['token']))) )
apjfnn.eval()
